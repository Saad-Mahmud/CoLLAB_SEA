...
"""
Example 05 — Run ICC_NM (natural communication + JSON) with vLLM or GPT (root)

What it shows
- How to generate an instance in-memory
- How to run ICC_NM (broadcast message phase) and sequential decisions
- vLLM/GPT selection with clear errors if config is missing

This variant reads provider settings from a JSON config to keep shell scripts simple.
"""
...
from communication_layer import ICCNaturalCommProtocol
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Example 05: Run ICC_NM with vLLM or GPT on a saved instance.")
    ...
    protocol = ICCNaturalCommProtocol(
        max_rounds=args.rounds,
        decision_max_tokens=256,
        decision_temperature=0.1,
        message_max_tokens=128,
        message_temperature=0.4,
        show_neighbour_assignments=show_neigh,
        show_factor_information=show_factor,
    )

    print(f"[Example05] Running ICC_NM | provider={args.provider} | rounds={args.rounds} | instance_dir={args.instance_dir}")
    result = protocol.run(
        instance.problem,
        agents,
        rounds=args.rounds,
        static_context={"note": "examples/example05_icc_nm"},
    )
