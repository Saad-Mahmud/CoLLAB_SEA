...
"""
Example 04 — Run ICC Image (vision-aware planning) with vLLM or GPT (root)

What it shows
- How to generate an image-enabled instance (per-agent collage/timeline/chart)
- How to run ICC image planning + JSON decision using ThinkingAgent
- How to attach images via static_context["image_map"]

Notes
- Personal Assistant and Meeting Scheduling enable images when instruction_mode=IMAGE
  and include_collages/include_timelines=True. Smart Grid enables include_charts=True.
- This example attaches exactly one image per agent when available.
"""
...
from communication_layer import ICCImageProtocol
...
def build_image_map(problem: str, instance: Any, root: Path) -> Dict[str, List[str]]:
    mapping: Dict[str, List[str]] = {}
    if problem == "personal_assistant":
        for aid, rel in (getattr(instance, "collages", {}) or {}).items():
            if rel:
                mapping[aid] = [str((root / rel).resolve())]
    elif problem == "meeting_scheduling":
        for aid, rel in (getattr(instance, "schedule_images", {}) or {}).items():
            if rel:
                mapping[aid] = [str((root / rel).resolve())]
    else:
        for aid, rel in (getattr(instance, "charts", {}) or {}).items():
            if rel:
                mapping[aid] = [str((root / rel).resolve())]
    return mapping
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Example 06: Run ICC Image with vLLM or GPT on a saved instance.")
    ...
    ap.add_argument("--config-mode", choices=("text", "image"), default="image", help="Which section of provider-config to use when present (vLLM)")
    ...
    image_map = build_image_map(problem_name, instance, inst_dir)
    if not image_map:
        print("[Warning] No images found for this instance; ICC Image will still run but without visual context.")

    show_neigh = problem_name != "smart_grid"
    show_factor = problem_name == "smart_grid"
    protocol = ICCImageProtocol(
        max_rounds=args.rounds,
        planning_max_tokens=384,
    ...
    print(f"[Example06] Running ICC Image | provider={args.provider} | rounds={args.rounds} | instance_dir={args.instance_dir}")
    result = protocol.run(
        instance.problem,
        agents,
        rounds=args.rounds,
        static_context={"note": "examples/example04_icc_image", "image_map": image_map},
    )
