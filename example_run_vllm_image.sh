...
# Example runner:
# 1) Generate instances via example00_generate.py (config-driven, images decided by config)
# 2) Start vLLM server in image mode using example02_run_vllm.py and configs/vllm_server.json
# 3) Run ICC Image for all three problems
# 4) Shut down the server gracefully

HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

PROBLEMS_CFG="configs/problems_image.json"
...
echo "[Step 2] Starting vLLM server via example02_run_vllm.py (image mode) ..."
# Best-effort: clear any stragglers to release ports
pkill -f 'api_layer/vllm_server.py' >/dev/null 2>&1 || true
pkill -f 'api_layer/vllm_server_image.py' >/dev/null 2>&1 || true

python example02_run_vllm.py --mode image --config "$VLLM_CFG" >/tmp/vllm_server_image.log 2>&1 &
WRAPPER_PID=$!
trap 'echo "[Info] Stopping vLLM wrapper (PID ${WRAPPER_PID})"; kill -INT ${WRAPPER_PID} >/dev/null 2>&1 || true' EXIT

echo "  -> PID=${WRAPPER_PID}; sleeping 120s for server warm-up (vision models take time) ..."
sleep 120
echo "  -> Continuing. If issues arise, check /tmp/vllm_server_image.log"
...
echo "[Step 3] Running ICC Image for all problems via vLLM ..."
for P in "${PROBLEMS[@]}"; do
  ...
  echo "--> ${P}: $INST_DIR"
  python example06_icc_image.py "$INST_DIR" --provider vllm --provider-config "$VLLM_CFG" --config-mode image --rounds 5 || {
    echo "[Warning] ICC Image failed for $P; see /tmp/vllm_server_image.log" >&2
  }
done
