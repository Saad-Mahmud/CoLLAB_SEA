...
def _load_pickled_instance(instance_dir: Path):
    pkls = sorted(instance_dir.glob("*.pkl"))
    if not pkls:
        raise FileNotFoundError(f"No pickled meeting scheduling instance in {instance_dir}")
...
def solve_instance(instance_dir: Path, config: SolverConfig) -> SolveResult:
    ...
    ms_instance = _load_pickled_instance(instance_dir)
    problem = ms_instance.problem
    ...
    max_utility = float(
        evaluation.get(
            "max_utility",
            getattr(ms_instance, "max_utility", total_utility),
        )
        or 0.0
    )
